...
public final class PartitionedWorkerExecutionPolicy implements MessageExecutionPolicy {
    ...
    private volatile TopicPartitionKeyExtractor topicExtractor;
    ...
    public PartitionedWorkerExecutionPolicy(
            ...
            ThreadFactory threadFactory,
            IdleStrategy idleStrategy) {
        this(runtime, dispatcher, extractor, null, null, config, threadFactory, idleStrategy);
    }

    /**
     * Constructs a partitioned-worker policy optionally wired for topic dispatch.
     *
     * @param runtime         the owning runtime
     * @param dispatcher      the generated message dispatcher
     * @param extractor       the message partition-key extractor (required)
     * @param topicDispatcher the generated topic dispatcher, or {@code null} for no topic dispatch
     * @param topicExtractor  the topic partition-key extractor, or {@code null} to route keyless topics
     *                        by topic id (preserves per-topic order on a single worker)
     * @param config          the partitioned execution configuration
     * @param threadFactory   the worker thread factory
     * @param idleStrategy    the worker idle strategy
     */
    public PartitionedWorkerExecutionPolicy(
            RingloomRuntime runtime,
            GeneratedMessageDispatcher dispatcher,
            PartitionKeyExtractor extractor,
            GeneratedTopicDispatcher topicDispatcher,
            TopicPartitionKeyExtractor topicExtractor,
            PartitionedExecutionConfig config,
            ThreadFactory threadFactory,
            IdleStrategy idleStrategy) {
    ...
    /** Wires the topic dispatcher and partition-key extractor after construction. */
    public void topicDispatcher(GeneratedTopicDispatcher topicDispatcher, TopicPartitionKeyExtractor topicExtractor) {
        this.topicExtractor = topicExtractor;
        for (Worker worker : workers) {
            worker.topicDispatcher(topicDispatcher);
        }
    }
    ...
    public int onTopicMessage(TopicMessage message, TopicContext ingressContext) {
        ...
        TopicPartitionKeyExtractor topicExtractor = this.topicExtractor;
        long key = topicExtractor != null ? topicExtractor.partitionKey(message, ingressContext) : message.topicId();
        Worker worker = workers[Math.floorMod(Long.hashCode(key), workers.length)];
        while (!closed.get()) {
            if (worker.writeTopic(message, ingressContext)) {
                return 1;
            }
            if (backpressurePolicy == WorkerBackpressurePolicy.FAIL_FAST) {
                return -1;
    ...
    private static final class Worker implements Runnable, AutoCloseable {
        ...
        private static final int TOPIC_MESSAGE_TYPE_ID = 2;
        ...
        // Topic-record header (distinct layout from service-message records).
        private static final int TOPIC_ID_OFFSET = 0; // long (8 bytes)
        private static final int TOPIC_INDEX_OFFSET = 8; // long (8 bytes)
        private static final int TOPIC_NAME_OFFSET = 16; // UTF-8 bytes (fixed 16)
        private static final int TOPIC_PAYLOAD_LENGTH_OFFSET = 32;
        private static final int TOPIC_PAYLOAD_OFFSET = 36;
        private static final int TOPIC_NAME_MAX_BYTES = 16;
        ...
        Worker(
            ...
            this.topicContext = new TopicContext(runtime);
        ...
        void topicDispatcher(GeneratedTopicDispatcher topicDispatcher) {
            this.topicDispatcher = topicDispatcher;
        }
        ...
        boolean writeTopic(TopicMessage message, TopicContext ingressContext) {
            RingloomRuntime ingressRuntime = ingressContext.runtime();
            if (runtime == null && ingressRuntime != null) {
                runtime = ingressRuntime;
            }
            long payloadLength = message.payloadSegment().byteSize();
            if (payloadLength > maxPayloadBytes) {
                throw new IllegalArgumentException("payload exceeds partitioned worker maxPayloadBytes");
            }
            int payloadBytes = Math.toIntExact(payloadLength);
            int recordLength = TOPIC_PAYLOAD_OFFSET + payloadBytes;
            int index = ringBuffer.tryClaim(TOPIC_MESSAGE_TYPE_ID, recordLength);
            if (index == RingBuffer.INSUFFICIENT_CAPACITY) {
                return false;
            }
            try {
                atomicBuffer.putLong(index + TOPIC_ID_OFFSET, message.topicId());
                atomicBuffer.putLong(index + TOPIC_INDEX_OFFSET, message.index());
                writeTopicName(index, ingressContext.topicName());
                atomicBuffer.putInt(index + TOPIC_PAYLOAD_LENGTH_OFFSET, payloadBytes);
                if (payloadBytes > 0) {
                    MemorySegment.copy(
                            message.payloadSegment(),
                            ValueLayout.JAVA_BYTE,
        ...
        private void writeTopicName(int index, String topicName) {
            for (int i = 0; i < TOPIC_NAME_MAX_BYTES; i++) {
                atomicBuffer.putByte(index + TOPIC_NAME_OFFSET + i, (byte) 0);
            }
            if (topicName == null) {
                return;
            }
            byte[] nameBytes = topicName.getBytes(java.nio.charset.StandardCharsets.UTF_8);
            int len = Math.min(nameBytes.length, TOPIC_NAME_MAX_BYTES);
            for (int i = 0; i < len; i++) {
                atomicBuffer.putByte(index + TOPIC_NAME_OFFSET + i, nameBytes[i]);
            }
        }
        ...
        private void dispatch(int messageTypeId, DirectBuffer buffer, int index, int length) {
            if (messageTypeId == MESSAGE_TYPE_ID) {
                dispatchServiceMessage(buffer, index, length);
            } else if (messageTypeId == TOPIC_MESSAGE_TYPE_ID) {
                dispatchTopicMessage(buffer, index, length);
            } else {
                throw new IllegalStateException("unknown partitioned worker message type " + messageTypeId);
            }
        }

        private void dispatchServiceMessage(DirectBuffer buffer, int index, int length) {
        ...
        private void dispatchTopicMessage(DirectBuffer buffer, int index, int length) {
            GeneratedTopicDispatcher topicDispatcher = this.topicDispatcher;
            if (topicDispatcher == null) {
                return;
            }
            int payloadLength = buffer.getInt(index + TOPIC_PAYLOAD_LENGTH_OFFSET);
            if (length != TOPIC_PAYLOAD_OFFSET + payloadLength) {
                throw new IllegalStateException("corrupt partitioned worker topic record length");
            }
            long topicId = buffer.getLong(index + TOPIC_ID_OFFSET);
            long messageIndex = buffer.getLong(index + TOPIC_INDEX_OFFSET);
            String topicName = readTopicName(index);
            MemorySegment payload = payloadLength == 0
                    ? MemorySegment.NULL
                    : ringSegment.asSlice(index + TOPIC_PAYLOAD_OFFSET, payloadLength);
            topicContext.updateCopied(topicId, topicName, messageIndex, payload);
            topicContext.runtime(runtime);
            topicDispatcher.onContextTopicMessage(topicContext);
        }

        private String readTopicName(int index) {
            int len = 0;
            byte[] bytes = new byte[TOPIC_NAME_MAX_BYTES];
            for (int i = 0; i < TOPIC_NAME_MAX_BYTES; i++) {
                byte b = atomicBuffer.getByte(index + TOPIC_NAME_OFFSET + i);
                if (b == 0) {
                    break;
                }
                bytes[len++] = b;
            }
            return len == 0 ? "" : new String(bytes, 0, len, java.nio.charset.StandardCharsets.UTF_8);
        }
